...
// Gauge sweep-up: hold at 0 for 1s, then ease every value from 0 to its target
// so the needles and numbers "spin up" like a car dashboard powering on.
    ...
    <div class="min-h-full max-w-6xl mx-auto px-4 md:px-8 py-6 flex flex-col gap-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Gauge
          ...
          color="#66d9ef"
          :markers="[
            { value: wpm.top5 * progress, color: '#facc15' },
            { value: wpm.top * progress, color: '#f92672' },
          ]"
        >
          <div class="grid grid-cols-3 gap-2 mt-2 text-center">
            <div><div class="text-[#66d9ef] text-xl font-bold">{{ r1(wpm.avg * progress) }}</div><div class="text-gray-500 text-[10px] tracking-widest">AVG</div></div>
            <div><div class="text-[#facc15] text-xl font-bold">{{ r1(wpm.top5 * progress) }}</div><div class="text-gray-500 text-[10px] tracking-widest">TOP 5%</div></div>
            <div><div class="text-[#f92672] text-xl font-bold">{{ r1(wpm.top * progress) }}</div><div class="text-gray-500 text-[10px] tracking-widest">BEST</div></div>
          </div>
        </Gauge>
        ...
        <Gauge
          label="ACCURACY"
          unit="percent"
          :value="acc.avg * progress"
          :min="50"
          :max="100"
          value-suffix="%"
          color="#a6e22e"
          :markers="[
            { value: acc.top5 * progress, color: '#facc15' },
            { value: acc.top * progress, color: '#66d9ef' },
          ]"
        >
          <div class="grid grid-cols-3 gap-2 mt-2 text-center">
            <div><div class="text-[#a6e22e] text-xl font-bold">{{ r1(acc.avg * progress) }}%</div><div class="text-gray-500 text-[10px] tracking-widest">AVG</div></div>
            <div><div class="text-[#facc15] text-xl font-bold">{{ r1(acc.top5 * progress) }}%</div><div class="text-gray-500 text-[10px] tracking-widest">TOP 5%</div></div>
            <div><div class="text-[#66d9ef] text-xl font-bold">{{ r1(acc.top * progress) }}%</div><div class="text-gray-500 text-[10px] tracking-widest">BEST</div></div>
          </div>
        </Gauge>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-[#1e1e1e] border border-gray-700 rounded-2xl p-5 flex flex-col items-center justify-center text-center">
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-5xl font-black text-white" style="font-family: 'Quantico', sans-serif">{{ Math.round(streak.current * progress) }}</span>
          </div>
          ...
          <div class="flex flex-col gap-3">
            <div v-for="g in growth" :key="g.days" class="flex items-center gap-3">
              ...
              <div class="flex-1 h-2.5 rounded-full bg-[#3a3a32] overflow-hidden">
                <div class="h-full rounded-full" :style="{ width: trend(g.pct).w * progress + '%', background: trend(g.pct).bar }"></div>
              </div>
